...
function nodecore.shelf_on_rightclick(pos, node, clicker, stack, pointed_thing)
...
function nodecore.shelf_stack_allow(pos, node, stack)
	node = node or minetest.get_node(pos)
	local def = minetest.registered_items[node.name]
	local idef = minetest.registered_items[stack:get_name()] or {}
	if idef.groups and idef.groups.container
	and idef.groups.container >= def.groups.shelf then return false end
end
...
nodecore.register_on_register_item(function(_, def)
		...
		if def.groups.shelf then
			def.groups.visinv = def.groups.visinv + 1
			def.groups.container = def.groups.container or def.groups.shelf
			...
			def.on_rightclick = def.on_rightclick or nodecore.shelf_on_rightclick
			def.on_punch = def.on_punch or nodecore.shelf_on_punch
			def.stack_allow = def.stack_allow or nodecore.shelf_stack_allow
		end
	end)
