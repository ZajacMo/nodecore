...
local super = nodecore.register_concrete
function nodecore.register_concrete(def)
	...
	local flow = basename .. "_wet_flowing"
	local src = basename .. "_wet_source"

	if def.craft_wet ~= false then
		nodecore.register_limited_abm({
				label = def.description .. " Wettening",
				interval = 1,
				chance = 2,
				limited_max = 100,
				nodenames = {basename},
		...
		nodecore.register_aism({
				label = def.description .. " Stack Wettening",
				interval = 1,
				chance = 2,
				itemnames = {basename},
	...
	if def.craft_wander ~= false then
		nodecore.register_limited_abm({
				label = def.description .. " Wandering",
				interval = 4,
				chance = 2,
				limited_max = 100,
				nodenames = {src},
	...
	if def.craft_sink ~= false then
		nodecore.register_limited_abm({
				label = def.description .. " Sink/Disperse",
				interval = 4,
				chance = 2,
				limited_max = 100,
				nodenames = {src},
	...
	if def.craft_cure ~= false then
		nodecore.register_craft({
				label = def.name .. " curing",
				action = "cook",
				duration = 300,
				cookfx = {smoke = 0.05},
				check = function(pos)
					return not minetest.find_node_near(pos,
						1, {flow, "group:water"})
				end,
				nodes = {
					{
						match = src,
						replace = def.to_molded
					}
				}
			})
